...
agent = load_agent(agent_paths[3])  # 加载模型
...
f = "SAC_NN_weights_bias.txt"  # 打开指定文本文件
numpy.set_printoptions(threshold=sys.maxsize)  # 用于设置文本输出数据的显示长度

with open(f, "w") as file:
    file.write('const float w1[sz_hid_1][sz_obs] = ')
    text = numpy.array2string(agent.pi.net[0].weight.detach().numpy(), separator=",")
    file.write(text.replace('[', '{').replace(']', '}') + ";\n\n")

    file.write("const float b1[sz_hid_1] = ")
    text = numpy.array2string(agent.pi.net[0].bias.detach().numpy(), separator=",")
    file.write(text.replace('[', '{').replace(']', '}') + ";\n\n")

    file.write('const float w2[sz_hid_2][sz_hid_1] = ')
    text = numpy.array2string(agent.pi.net[2].weight.detach().numpy(), separator=",")
    file.write(text.replace('[', '{').replace(']', '}') + ";\n\n")

    file.write('const float b2[sz_hid_2] = ')
    text = numpy.array2string(agent.pi.net[2].bias.detach().numpy(), separator=",")
    file.write(text.replace('[', '{').replace(']', '}') + ";\n\n")

    file.write('const float w3[sz_act][sz_hid_2] = ')
    text = numpy.array2string(agent.pi.mu_layer.weight.detach().numpy(), separator=",")
    file.write(text.replace('[', '{').replace(']', '}') + ";\n\n")

    file.write('const float b3[sz_act] = ')
    text = numpy.array2string(agent.pi.log_std_layer.bias.detach().numpy(), separator=",")
    file.write(text.replace('[', '{').replace(']', '}') + ";\n\n")
