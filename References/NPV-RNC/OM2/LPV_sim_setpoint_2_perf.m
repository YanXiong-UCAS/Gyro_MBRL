...
for k=1:L+1
    x(:,k)=Xg(xs); q2(k)=Xg(2);
    mu(:,k)=LPV_kff_2(Xg,xr(:,k),ur(:,k),p,Fv,Km);
...
DS=2;
LPV_setpoint_2.t=downsample(t,DS);
LPV_setpoint_2.x=downsample(x',DS)';
LPV_setpoint_2.xr=downsample(xr',DS)';
LPV_setpoint_2.q2=downsample(q2,DS);
LPV_setpoint_2.zs=downsample(zs,DS);
LPV_setpoint_2.zsi=zsi;
save('LPV_setpoint_2_perf.mat','LPV_setpoint_2');
