...
x1_diff_int and x3_diff_int are computed differently. They are not normalized in this version, see line 233-234
'''
...
class GyroscopeIntegralEnvV1(gym.Env):
    ...
    def init(self, simu_args = {}, reward_func = 'Quadratic', reward_args = {}):
        ...
        self.state_bound = np.array([self.maxAngle, self.maxGimbalSpeed, self.maxAngle, self.maxGimbalSpeed, 
                                   self.maxAngle, self.maxAngle, self.maxDiskSpeed, np.inf, np.inf], dtype = np.float32)
        self.state_space = spaces.Box(low = -self.state_bound, high = self.state_bound, dtype = np.float32)
        
        # Observation space (normalized), 11D
    ...
    def step(self, a):
        ...
        x1, x2, x3, x4, x1_ref, x3_ref, w, x1_diff_int, x3_diff_int = self.state
        a1, a2 = a
        ...
        self.state = np.array([x1, x2, x3, x4, x1_ref, x3_ref, w, x1_diff_int, x3_diff_int])
        self.observe()
    ...
    def quad_reward(self, x1_diff, x3_diff, x2, x4, dx2, dx4, x1_diff_int, x3_diff_int, u1, u2, qx1 = 1, qx2 = 0.01, qx3 = 1, qx4 = 0.01, qi1 = 0.01, qi3 = 0.01, pu1 = 0, pu2 = 0):
        return -(qx1*(x1_diff**2) + qx3*(x3_diff**2) + qx2*(x2**2) + qx4*(x4**2) + qi1*(x1_diff_int**2) + qi3*(x3_diff_int**2) + pu1*(u1**2) + pu2*(u2**2))
    ...
    def sparse_reward(self, x1_diff, x3_diff, x2, x4, dx2, dx4, u1, u2, bx = 0.01, rx = 1, bv = 0.01, rv = 1, bu = 0.01, ru = 1):
        r = 0
    ...
    def observe(self):
        ...
        self.observation = np.array([np.cos(s[0]), np.sin(s[0]), s[1]/self.maxGimbalSpeed, 
                                     ...
                                     np.sign(s[7])*abs(s[7])/(1+abs(s[7])), 
                                     np.sign(s[8])*abs(s[8])/(1+abs(s[8]))])
        return self.observation
